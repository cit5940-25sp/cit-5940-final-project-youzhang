...
async function searchMovies() {
    try {
        ...
        // Send search request with autocomplete
        ...
        elements.searchResults.innerHTML = '<p class="error">Error searching movies</p>';
    }
}
...
function displaySearchResults(movies) {
    ...
    if (!movies || movies.length === 0) {
        elements.searchResults.innerHTML = '<p class="no-results">No matching movies found</p>';
        return;
    }
    ...
    movies.forEach(movie => {
        ...
        movieElement.innerHTML = `
            ...
            <button class="select-movie-btn">Select</button>
        `;
        
        // Add click event to the select button
        const selectButton = movieElement.querySelector('.select-movie-btn');
        selectButton.addEventListener('click', () => selectMovie(movie.id));
        
        // Add click event to the entire movie item
    ...
    elements.searchResults.appendChild(resultsContainer);
}
